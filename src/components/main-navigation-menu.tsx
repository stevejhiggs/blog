    ...
    <NavigationMenu>
      <NavigationMenuList>
        {links ? (
          <NavigationMenuItem>
            {links.map((link) => (
              <a key={link.href} href={link.href} className={navigationMenuTriggerStyle()} {...(link.forceReload ? { 'data-astro-reload': true } : {})}>
                {link.title}
              </a>
            ))}
          </NavigationMenuItem>
...
const ListItem: React.FC<MenuItem> = ({ title, href, description, launched, disabled, external, forceReload }) => {
    ...
    <li>
      ...
      >
        <div className="flex items-center text-sm font-medium leading-none">
          ...
          {launched ? (
            <Badge radius="sm" className="h-5 px-1.5 text-xs font-medium bg-[#ebf5ff] hover:bg-[#ebf5ff] text-[#0068d6]">
              NEW
            </Badge>
          ) : null}
        </div>
